<template>
 ...
 -->
		<view class="section">
			<scroll-view v-if="list && list.length >0" :scroll-y="true" :lower-threshold="100"
				:scroll-with-animation="true" class="scl" @scrolltolower="onScroll2bottom">
				<view class="itm" v-for="(itm, idx) in list" :key="idx" @click="goDetail(itm)">
					<view class="pic-cot">
						<view class="itm-logo" :class="itm['名称拼音']"></view>
					</view>
					<view class="cotxt">
						<view class="main-info">
							...
							<view class="property gry">属性：
								<view class="pro-spe" v-for="(p, x) in getProp(itm['属性'])" :key="x"
									:style="{'background-color': p.color}">{{p.name}}</view>
							</view>
							<view class="character gry">特性：{{itm['特性']}}</view>
						</view>
						<view class="nom">
							No.{{itm['帕底亚图鉴']<10?('00'+itm['帕底亚图鉴']):(itm['帕底亚图鉴']<100?('0'+itm['帕底亚图鉴']):itm['帕底亚图鉴'])}}
						</view>
					</view>
				</view>
			</scroll-view>
...
<script>
	export default {
		...
		created() {
			...
			let _this = this
			uni.downloadFile({
				url: 'https://636c-cloud1-8gxep97n51873-4bo66862e73-1316653034.tcb.qcloud.la/bkm/msp-sv.webp?sign=e0229137f2890fa57d0ca50a0f1a45f6&t=1674967884',
